// Package jwt provides JWT management and token claim validation helpers.
package jwt

import (
	"time"

	"blackradar/api/shared"
...
func (s *JWTManager) generateToken(username string, tokenID string, scope string, tokenUse string, expiration time.Duration) (string, error) {
	if len(s.secret) == 0 {
		return "", shared.ErrMissingSecret
	}
...
func (s *JWTManager) extractClaims(tokenString string, scope string, tokenUse string) (AccessClaims, error) {
	if len(s.secret) == 0 {
		return AccessClaims{}, shared.ErrMissingSecret
	}

	claims := &AccessClaims{}
	token, err := jwt.ParseWithClaims(tokenString, claims, func(token *jwt.Token) (any, error) {
		if token.Method != jwt.SigningMethodHS256 {
			return nil, shared.ErrUnexpectedSigningMethod
		}
		return s.secret, nil
	...
	if err != nil || !token.Valid {
		return AccessClaims{}, shared.ErrInvalidToken
	}
	if claims.Subject == "" {
		return AccessClaims{}, shared.ErrMissingSubject
	}
	if claims.Scope != scope {
		return AccessClaims{}, shared.ErrInvalidScope
	}
	if claims.TokenUse != tokenUse {
		return AccessClaims{}, shared.ErrInvalidTokenUse
	}
	return *claims, nil
}
