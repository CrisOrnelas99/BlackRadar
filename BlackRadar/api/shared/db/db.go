...
func TranslateDatabaseError(err error) error {
	...
	case isPostgresError(err, "23503"):
		return fmt.Errorf("%w: %w", shared.ErrForeignKeyViolation, err)
	case isPostgresError(err, "23514"):
		return fmt.Errorf("%w: %w", shared.ErrCheckConstraintViolation, err)
	case isPostgresError(err, "23505"):
		return fmt.Errorf("%w: %w", shared.ErrUniqueViolation, err)
	default:
		return err
	}
}
