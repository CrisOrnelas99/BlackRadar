...
func TestBuildAssetMatchRankingRequestLocksSystemPrompt(t *testing.T) {
	request := BuildAssetMatchRankingRequest("fingerprint", "dell latitude 7420", []cpeclient.CPECandidate{
...
func TestBuildAssetMatchRankingRequestCapsCandidates(t *testing.T) {
	...
	for i := range candidates {
		candidates[i] = cpeclient.CPECandidate{CPEName: "cpe:2.3:a:dell:latitude_7420:*:*:*:*:*:*:*:*", Title: "candidate"}
	}
...
func TestBuildAssetCVERankingRequestLocksSystemPrompt(t *testing.T) {
	request := BuildAssetCVERankingRequest("product=wp ultimate map;version=1.1", []string{"wp ultimate map"}, []cveclient.CVELookupResponse{
...
func TestBuildAssetCVERankingRequestCapsCandidates(t *testing.T) {
	candidates := make([]cveclient.CVELookupResponse, 25)
	for i := range candidates {
		candidates[i] = cveclient.CVELookupResponse{CVEID: "CVE-2026-12345", Title: "candidate"}
	}
