...
- **Who:** Alice is authenticated; Bob owns a different asset.
- **What:** Alice requests an asset by identifier.
- **When:** The API receives the request after authentication middleware runs.
- **Where:** The controller binds the request, the service applies the ownership rule, and the repository filters the database query.
- **Why:** An identifier is not permission. Alice must not read Bob's record by guessing its ID.
- **How:** The server derives Alice's identity from authentication context and requires both the asset ID and Alice's owner ID in the lookup.
