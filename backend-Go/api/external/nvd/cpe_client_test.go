// Package nvd verifies the CPE search client behavior.
package nvd
...
func TestCPEClientSearchCandidates(t *testing.T) {
	...
	client, err := NewCPEClientWithHTTPClient(server.URL+"/rest/json/cpes/2.0", "test-key", server.Client(), NewRateLimiter(10, 30*time.Second))
	if err != nil {
